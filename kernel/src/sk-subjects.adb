...
is
   ...
   with
      ...
      Refined_Post   => Accepts_Interrupts'Result =
         (Descriptors (ID).State.Intr_State = 0 and then
            Bitops.Bit_Test (Value => Descriptors (ID).State.RFLAGS,
                             Pos   => Constants.RFLAGS_IF_FLAG))
   is
   begin
      return Descriptors (ID).State.Intr_State = 0
        and then Bitops.Bit_Test
          (Value => Descriptors (ID).State.RFLAGS,
           Pos   => Constants.RFLAGS_IF_FLAG);
   ...
   begin
      ...
      Ctx.Descriptor := Descriptors (ID).State;
   end Create_Context;
   ...
   is
   begin
      Descriptors (ID).State.CR4 := Bitops.Bit_Set
        (Value => Descriptors (ID).State.CR4,
   ...
   with
      ...
      Refined_Post    => Descriptors (ID).State.RIP =
     Descriptors (ID).State.RIP'Old + Word64
     (Descriptors (ID).State.Instruction_Len)
   is
      Next_RIP : constant Word64 := Descriptors (ID).State.RIP +
                   Word64 (Descriptors (ID).State.Instruction_Len);
   begin
      Descriptors (ID).State.RIP := Next_RIP;
   end Increment_RIP;

   -------------------------------------------------------------------------

   function Is_CPL_0 (ID : Skp.Global_Subject_ID_Type) return Boolean
   is ((Descriptors (ID).State.Segment_Regs.CS.Selector
        and SEGMENT_SELECTOR_PL_MASK) = 0)
   with
      Refined_Global => (Input => Descriptors),
      Refined_Post   => Is_CPL_0'Result =
       ((Descriptors (ID).State.Segment_Regs.CS.Selector
        and SEGMENT_SELECTOR_PL_MASK) = 0);
   ...
   is
      Value : Word64;
   begin
      Descriptors (ID).State.Exit_Reason := Word32'Mod (Exit_Reason);
      VMX.VMCS_Read (Field => Constants.VMX_EXIT_QUALIFICATION,
                     Value => Descriptors (ID).State.Exit_Qualification);
      VMX.VMCS_Read (Field => Constants.GUEST_INTERRUPTIBILITY,
                     Value => Value);
      Descriptors (ID).State.Intr_State := Word32'Mod (Value);
      VMX.VMCS_Read (Field => Constants.VMX_EXIT_INSTRUCTION_LEN,
                     Value => Value);
      Descriptors (ID).State.Instruction_Len := Word32'Mod (Value);

      VMX.VMCS_Read (Field => Constants.GUEST_PHYSICAL_ADDRESS,
                     Value => Descriptors (ID).State.Guest_Phys_Addr);
      ...
      VMX.VMCS_Read (Field => Constants.GUEST_CR3,
                     Value => Descriptors (ID).State.CR3);
      ...
      VMX.VMCS_Read (Field => Constants.GUEST_LIMIT_GDTR,
                     Value => Value);
      Descriptors (ID).State.GDTR.Limit := Word32'Mod (Value);
      VMX.VMCS_Read (Field => Constants.GUEST_BASE_IDTR,
                     Value => Descriptors (ID).State.IDTR.Base);
      VMX.VMCS_Read (Field => Constants.GUEST_LIMIT_IDTR,
                     Value => Value);
      Descriptors (ID).State.IDTR.Limit := Word32'Mod (Value);

      VMX.VMCS_Read (Field => Constants.GUEST_SYSENTER_CS,
                     Value => Value);
      Descriptors (ID).State.SYSENTER_CS := Word32'Mod (Value);
      ...
      Descriptors (ID).State.Regs := Regs;
   end Save_State;
   ...
   is
     (Bitops.Bit_Test (Value => Descriptors (ID).State.CR4,
                       Pos   => Constants.CR4_MCE_FLAG));
